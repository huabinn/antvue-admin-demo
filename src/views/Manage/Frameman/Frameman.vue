		...
		<a-card :bordered="false" :loading="loading">
			...
			<div class="card-content">
				<div class="button-group">
					...
					<a-button type="primary" style="width: 88px" @click="newFrame">新增</a-button>
				</div>
				<a-table
				...
				>
					<template #operation="{ record }">
						<a-space class="operate">
							<a @click="editFrame(record.key)">编辑</a>
...
export default defineComponent({
	...
	setup() {
		 ...
		 */
		const newFrame = () => { 
			router.push("/manage/frameman/newframe")
		}
		/**
		 * 编辑发放
		 * @param {number} key 
		...
		const editFrame = key => { 
			console.log('key', key);
			router.push("/manage/frameman/editframe")
		}
		/**
		 * 暂停操作
		 * @param {number} key 
